...
class CodeTool:
    """Writes code content within the configured output directory."""

    def __init__(self, output_dir: Path) -> None:
        self._output_dir = output_dir.resolve()

    def write(self, payload: ActionPayload) -> ToolExecutionResult:
        filename = payload.filename
        content = payload.content
        if not filename:
            raise ToolExecutionError("Filename is required for code generation.")
        if content is None:
            raise ToolExecutionError("Content is required for code generation.")
        ...
        return ToolExecutionResult(
            action="write_code",
            status="completed",
            message=f"Wrote code to {destination}",
    ...
    def _resolve_output_path(self, filename: str) -> Path:
        ...
        if self._output_dir not in candidate.parents:
            raise ToolExecutionError("Code writes are restricted to the output directory.")
        return candidate
